...
parser.add_argument('--data_root', type=str, default="/tanData")
...
if args.data_path is None:
    data_prefix = Path(data_path(args.dataset, args.data_root))
else:
...
if args.dataset == 'iwslt-ende' or args.dataset == 'iwslt-deen':
    ...
    load_dataset=args.load_dataset, save_dataset=args.save_dataset, prefix='normal') \
        if args.mode in ["train", "test", "distill"] else None
...
if args.dataset != "mscoco":
    ...
    if args.num_gpus == 1:
        train_real = data.BucketIterator(train_data, args.batch_size, device=torch.device(args.gpu), batch_size_fn=batch_size_fn,
                                        train=train_flag, repeat=train_flag, shuffle=train_flag) if not train_data is None else None
        dev_real = data.BucketIterator(dev_data, args.batch_size, device=torch.device(args.gpu), batch_size_fn=batch_size_fn,
                                        train=False, repeat=False, shuffle=False) if not dev_data is None else None
        test_real = data.BucketIterator(test_data, args.batch_size, device=torch.device(args.gpu), batch_size_fn=batch_size_fn,
