...
def positional_encodings_like(x, t=None):   # hope to be differentiable
    ...
    encodings = positions.unsqueeze(-1) @ channels.unsqueeze(0).to(x)  # batch x target_len x 256
...
class MomentumAttention(nn.Module):
    ...
    def __init__(self, drop_ratio, causal, mu, stepsize, eps=1e-6):
        super().__init__()
...
#             key *= mask[:, :, None]
        ...
        L = value.shape[1]
        momentum_weight = (self.stepsize * (1.0 - torch.pow(torch.ones(L) * self.mu, L - torch.from_numpy(np.arange(1,L+1)) + 1))/(1.0 - self.mu)).to(value)
        value = value *  momentum_weight[None, :, None]

#         KV = torch.einsum("nsd,nsm->ndm", key, value)
#         Z = 1./(torch.einsum("nsd,nd->ns", query, key.sum(dim=1))+self.eps)
#         V = torch.einsum("nsd,ndm,ns->nsm", query, KV, Z)

        KV = matmul(key.transpose(1, 2), value)
        Z = 1./(matmul(query, key.sum(dim=1)) + self.eps)

        return matmul(query, KV) * Z[:, :, None]
    
class LinearAttention(nn.Module):

    def __init__(self, drop_ratio, causal, eps=1e-6):
        super().__init__()
        # self.dropout = nn.Dropout(drop_ratio)
        #self.causal = causal
        self.eps = eps
...
class MultiHead2(nn.Module):
    ...
    def __init__(self, d_key, d_value, n_heads, drop_ratio,
                causal=False, diag=False, use_wo=True, attn_type='full', name=None, mu=0.0, stepsize=1.1):
        super().__init__()
        if attn_type == 'linear':
            self.attention = LinearAttention(drop_ratio, causal=causal)
        elif attn_type == 'momentum':
            self.attention = MomentumAttention(drop_ratio, causal=causal, mu=mu, stepsize=stepsize)
        else:
    ...
    def forward(self, query, key, value, mask=None, feedback=None, weights=None, beta=0, tau=1):
        ...
        if self.attn_type == 'linear' or self.attn_type == 'momentum':
            query = torch.nn.functional.elu(query) + 1
            key = torch.nn.functional.elu(key) + 1
            outputs = self.attention(query, key, value, mask)  # (B x N) x T x (D/N)
        else:
        ...
        if (feedback is not None) and (self.attn_type == 'full'):
...
class ResidualMomentumBlock(nn.Module):

    def __init__(self, layer, d_model, d_hidden, drop_ratio, pos=0, res_mu=0.0, res_stepsize=1.0):
        super().__init__()
        self.layer = layer
        self.dropout = nn.Dropout(drop_ratio)
        self.layernorm = LayerNorm(d_model)
        ...
        # for momentum residual connections
        self.res_mu = res_mu
        self.res_stepsize = res_stepsize

    def forward(self, v, *x):
        vy = self.res_mu * v - self.dropout(self.layer(*x))

        return vy, self.layernorm(x[self.pos] - self.res_stepsize * vy)
...
class ResidualAdaptiveBlock(nn.Module):
    ...
    def __init__(self, layer, d_model, d_hidden, drop_ratio, pos=0, res_stepsize=1.0, res_delta=0.0001, adaptive_type="wang"):
        ...
        # for adaptive residual connections
        self.res_stepsize = res_stepsize
        self.res_delta = res_delta
        self.adaptive_type = adaptive_type

    def forward(self, v, gradval, *x):
        gradvaly = self.dropout(self.layer(*x))
        if self.adaptive_type == "fr":
            #Fletcher–Reeves
            res_mu = (torch.norm(gradvaly, dim=2, keepdim=True)/torch.norm(gradval, dim=2, keepdim=True))**2
        elif self.adaptive_type == "pr":
            #Polak–Ribière
            res_mu = torch.sum(gradvaly * (gradvaly - gradval), dim=2, keepdim=True)/(torch.norm(gradval, dim=2, keepdim=True)**2)
        elif self.adaptive_type == "hs":
            #Hestenes-Stiefel
            if torch.all(v == torch.zeros_like(x[self.pos])):
                res_mu = torch.sum(gradvaly * (gradvaly - gradval), dim=2, keepdim=True)/torch.sum(gradvaly - gradval, dim=2, keepdim=True)
            else:
                res_mu = torch.sum(gradvaly * (gradvaly - gradval), dim=2, keepdim=True)/torch.sum(v * (gradvaly - gradval), dim=2, keepdim=True)
        elif self.adaptive_type == "dy":
            #Dai-Yuan
            if torch.all(v == torch.zeros_like(x[self.pos])):
                res_mu = torch.sum(gradvaly * (gradvaly - gradval), dim=2, keepdim=True)/torch.sum(gradvaly - gradval, dim=2, keepdim=True)
            else:
        ...
        return vy, gradvaly, self.layernorm(x[self.pos] - self.res_stepsize * vy)
...
class HighwayBlock(nn.Module):
    ...
    def forward(self, *x):
        g = torch.sigmoid(self.gate(x[self.pos]))
    ...
    def __init__(self, args):
        ...
        self.selfattn = ResidualBlock(
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                       args.drop_ratio, use_wo=args.use_wo, attn_type=args.attn_type, name='encoder_self_attn', mu=args.mu, stepsize=args.stepsize),
...
class EncoderMomentumLayer(nn.Module):

    def __init__(self, args):
        super().__init__()
        self.selfattn = ResidualMomentumBlock(
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                       args.drop_ratio, use_wo=args.use_wo, attn_type=args.attn_type, name='encoder_self_attn', mu=args.mu, stepsize=args.stepsize),
            args.d_model, args.d_hidden, args.drop_ratio, res_mu=args.res_mu, res_stepsize=args.res_stepsize)
        self.feedforward = args.block_cls(
            FeedForward(args.d_model, args.d_hidden),
            args.d_model, args.d_hidden, args.drop_ratio )

    def forward(self, v, x, mask=None):
        v, x = self.selfattn(v, x, x, x, mask)
        x = self.feedforward(x)
        return v, x

    
class EncoderAdaptiveLayer(nn.Module):

    def __init__(self, args):
        super().__init__()
        self.selfattn = ResidualAdaptiveBlock(
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                       args.drop_ratio, use_wo=args.use_wo, attn_type=args.attn_type, name='encoder_self_attn', mu=args.mu, stepsize=args.stepsize),
            args.d_model, args.d_hidden, args.drop_ratio, res_stepsize=args.res_stepsize, res_delta=args.res_delta, adaptive_type=args.adaptive_type)
        self.feedforward = args.block_cls(
            FeedForward(args.d_model, args.d_hidden),
            args.d_model, args.d_hidden, args.drop_ratio )

    def forward(self, v, gradval, x, mask=None):
        v, gradval, x = self.selfattn(v, gradval, x, x, x, mask)
        x = self.feedforward(x)
        return v, gradval, x
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, args, causal=True, diag=False,
        ...
        self.selfattn = ResidualBlock(
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                    ...
                    use_wo=args.use_wo, attn_type=args.attn_type, name='decoder_self_attn', mu=args.mu, stepsize=args.stepsize),
            args.d_model, args.d_hidden, args.drop_ratio)

        self.attention = ResidualBlock(
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                    args.drop_ratio, use_wo=args.use_wo, attn_type=args.attn_type, name='decoder_source_attn', mu=args.mu, stepsize=args.stepsize),
            args.d_model, args.d_hidden, args.drop_ratio)
        ...
        if positional:
            ...
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                    args.drop_ratio, causal=causal, diag=diag,
                    use_wo=args.use_wo, attn_type=args.attn_type, name='pos_source_attn', mu=args.mu, stepsize=args.stepsize),
    ...
    def forward(self, x, encoding, p=None, mask_src=None, mask_trg=None, feedback=None):
        ...
        if self.attn_type == 'linear' or self.attn_type == 'momentum':
            x = self.attention(x, encoding, encoding, mask_src)
        else:
...
class DecoderMomentumLayer(nn.Module):
    ...
    def __init__(self, args, causal=True, diag=False,
        ...
        self.attention = ResidualMomentumBlock(
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                    args.drop_ratio, use_wo=args.use_wo, attn_type=args.attn_type, name='decoder_source_attn', mu=args.mu, stepsize=args.stepsize),
            args.d_model, args.d_hidden, args.drop_ratio, res_mu=args.res_mu, res_stepsize=args.res_stepsize)

        if positional:
            self.pos_selfattn = ResidualMomentumBlock(
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                    args.drop_ratio, causal=causal, diag=diag,
                    use_wo=args.use_wo, attn_type=args.attn_type, name='pos_source_attn', mu=args.mu, stepsize=args.stepsize),
            args.d_model, args.d_hidden, args.drop_ratio, pos=2, res_mu=args.res_mu, res_stepsize=args.res_stepsize)
    ...
    def forward(self, v, x, encoding, p=None, mask_src=None, mask_trg=None, feedback=None):

        feedback_src = []
        feedback_trg = []
        
        if self.attn_type == 'linear' or self.attn_type == 'momentum':
            v, x = self.selfattn(v, x, x, x, mask_trg)   #
        else:
            v, x = self.selfattn(v, x, x, x, mask_trg, feedback_trg)   #

        if self.positional:
            pos_encoding, weights = positional_encodings_like(x), None
            if self.attn_type == 'linear' or self.attn_type == 'momentum':
                v, x = self.pos_selfattn(v, pos_encoding, pos_encoding, x, mask_trg)  # positional attention
            else:
                v, x = self.pos_selfattn(v, pos_encoding, pos_encoding, x, mask_trg, None, weights)  # positional attention
        
        if self.attn_type == 'linear' or self.attn_type == 'momentum':
            v, x = self.attention(v, x, encoding, encoding, mask_src)
        else:
            v, x = self.attention(v, x, encoding, encoding, mask_src, feedback_src)
...
class DecoderAdaptiveLayer(nn.Module):
    ...
    def __init__(self, args, causal=True, diag=False,
        ...
        self.attention = ResidualAdaptiveBlock(
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                    args.drop_ratio, use_wo=args.use_wo, attn_type=args.attn_type, name='decoder_source_attn', mu=args.mu, stepsize=args.stepsize),
            args.d_model, args.d_hidden, args.drop_ratio, res_stepsize=args.res_stepsize, res_delta=args.res_delta, adaptive_type=args.adaptive_type)

        if positional:
            self.pos_selfattn = ResidualAdaptiveBlock(
            MultiHead2(args.d_model, args.d_model, args.n_heads,
                    args.drop_ratio, causal=causal, diag=diag,
                    use_wo=args.use_wo, attn_type=args.attn_type, name='pos_source_attn', mu=args.mu, stepsize=args.stepsize),
            args.d_model, args.d_hidden, args.drop_ratio, pos=2, res_stepsize=args.res_stepsize, res_delta=args.res_delta, adaptive_type=args.adaptive_type)
    ...
    def forward(self, v, gradval, x, encoding, p=None, mask_src=None, mask_trg=None, feedback=None):

        feedback_src = []
        feedback_trg = []
        
        if self.attn_type == 'linear' or self.attn_type == 'momentum':
            v, gradval, x = self.selfattn(v, gradval, x, x, x, mask_trg)   #
        else:
            v, gradval, x = self.selfattn(v, gradval, x, x, x, mask_trg, feedback_trg)   #

        if self.positional:
            pos_encoding, weights = positional_encodings_like(x), None
            if self.attn_type == 'linear' or self.attn_type == 'momentum':
                v, gradval, x = self.pos_selfattn(v, gradval, pos_encoding, pos_encoding, x, mask_trg)  # positional attention
            else:
                v, gradval, x = self.pos_selfattn(v, gradval, pos_encoding, pos_encoding, x, mask_trg, None, weights)  # positional attention
        
        if self.attn_type == 'linear' or self.attn_type == 'momentum':
            v, gradval, x = self.attention(v, gradval, x, encoding, encoding, mask_src)
        else:
            v, gradval, x = self.attention(v, gradval, x, encoding, encoding, mask_src, feedback_src)
...
class EncoderMomentum(nn.Module):
    ...
    def __init__(self, field, args):
        ...
        if args.dataset != "mscoco":
            if args.share_embed:
                ...
                self.embed = nn.Embedding(len(field.vocab), args.d_model)
        self.layers = nn.ModuleList(
            [EncoderMomentumLayer(args) for i in range(args.n_layers)])
        self.dropout = nn.Dropout(args.input_drop_ratio)
        if args.dataset != "mscoco":
            self.field = field
        self.d_model = args.d_model
...
class EncoderAdaptive(nn.Module):
    ...
    def __init__(self, field, args):
        ...
        if args.dataset != "mscoco":
            if args.share_embed:
                ...
                self.embed = nn.Embedding(len(field.vocab), args.d_model)
        self.layers = nn.ModuleList(
            [EncoderAdaptiveLayer(args) for i in range(args.n_layers)])
        self.dropout = nn.Dropout(args.input_drop_ratio)
        if args.dataset != "mscoco":
            self.field = field
        self.d_model = args.d_model
    ...
    def forward(self, x, mask=None):
        ...
        for layer in self.layers:
            v, gradval, x = layer(v, gradval, x, mask)
            encoding.append(x)
        return encoding
...
class Decoder(nn.Module):
    ...
    def greedy(self, encoding, mask_src=None, mask_trg=None, feedback=None):
        ...
        if (feedback is not None) and (args.attn_type == 'full'):
...
class DecoderMomentum(nn.Module):
    ...
    def __init__(self, field, args, causal=True, positional=False, diag=False, out=None):
        ...
        self.layers = nn.ModuleList(
            [DecoderMomentumLayer(args, causal, diag, positional)
            for i in range(args.n_layers)])
    ...
    def forward(self, x, encoding, source_masks=None, decoder_masks=None,
        ...
        if self.enc_last:
            for l, layer in enumerate(self.layers):
                v, x = layer(v, x, encoding[-1], mask_src=source_masks, mask_trg=decoder_masks, feedback=feedback)
        else:
            for l, (layer, enc) in enumerate(zip(self.layers, encoding[1:])):
                v, x = layer(v, x, enc, mask_src=source_masks, mask_trg=decoder_masks, feedback=feedback)
        return x
    ...
    def greedy(self, encoding, mask_src=None, mask_trg=None, feedback=None):
        ...
        for t in range(T):
            ...
            inter_attention = []
            v = torch.zeros_like(hiddens[0][:, :t+1])
            for l in range(len(self.layers)):
                x = hiddens[l][:, :t+1]
                v, x = self.layers[l].selfattn(v, hiddens[l][:, t:t+1], x, x)   # we need to make the dimension 3D
                v, out_temp = self.layers[l].attention(v, x, encoding[l], encoding[l], mask_src, inter_attention)
                hiddens[l + 1][:, t] = self.layers[l].feedforward(out_temp)[:, 0]
    ...
    def beam_search(self, encoding, mask_src=None, mask_trg=None, width=2, alpha=0.6):  # width: beamsize, alpha: length-norm
        ...
        for t in range(T):
            hiddens[0][:, :, t] = self.dropout(
                hiddens[0][:, :, t] + F.embedding(outs[:, :, t], embedW))
            v = torch.zeros_like(hiddens[0][:, :, :t + 1].contiguous().view(B * W, -1, C))
            for l in range(len(self.layers)):
                x = hiddens[l][:, :, :t + 1].contiguous().view(B * W, -1, C)
                v, x = self.layers[l].selfattn(v, x[:, -1:, :], x, x)
                v, out_temp = self.layers[l].attention(v, x, encoding[l], encoding[l], mask_src)
                hiddens[l + 1][:, :, t] = self.layers[l].feedforward(out_temp).view(B, W, C)
...
class DecoderAdaptive(nn.Module):
    ...
    def __init__(self, field, args, causal=True, positional=False, diag=False, out=None):
        ...
        self.layers = nn.ModuleList(
            [DecoderAdaptiveLayer(args, causal, diag, positional)
            for i in range(args.n_layers)])
    ...
    def forward(self, x, encoding, source_masks=None, decoder_masks=None,
        ...
        if self.enc_last:
            for l, layer in enumerate(self.layers):
                v, gradval, x = layer(v, gradval, x, encoding[-1], mask_src=source_masks, mask_trg=decoder_masks, feedback=feedback)
        else:
            for l, (layer, enc) in enumerate(zip(self.layers, encoding[1:])):
                v, gradval, x = layer(v, gradval, x, enc, mask_src=source_masks, mask_trg=decoder_masks, feedback=feedback)
        return x
    ...
    def greedy(self, encoding, mask_src=None, mask_trg=None, feedback=None):
        ...
        for t in range(T):
            ...
            inter_attention = []
            v = torch.zeros_like(hiddens[0][:, :t+1])
            gradval = torch.ones_like(hiddens[0][:, :t+1])
            for l in range(len(self.layers)):
                x = hiddens[l][:, :t+1]
                v, gradval, x = self.layers[l].selfattn(v, gradval, hiddens[l][:, t:t+1], x, x)   # we need to make the dimension 3D
                v, gradval, out_temp = self.layers[l].attention(v, gradval, x, encoding[l], encoding[l], mask_src, inter_attention)
                hiddens[l + 1][:, t] = self.layers[l].feedforward(out_temp)[:, 0]
    ...
    def beam_search(self, encoding, mask_src=None, mask_trg=None, width=2, alpha=0.6):  # width: beamsize, alpha: length-norm
        ...
        for t in range(T):
            hiddens[0][:, :, t] = self.dropout(
                hiddens[0][:, :, t] + F.embedding(outs[:, :, t], embedW))
            v = torch.zeros_like(hiddens[0][:, :, :t + 1].contiguous().view(B * W, -1, C))
            gradval = torch.ones_like(hiddens[0][:, :, :t + 1].contiguous().view(B * W, -1, C))
            for l in range(len(self.layers)):
                x = hiddens[l][:, :, :t + 1].contiguous().view(B * W, -1, C)
                v, gradval, x = self.layers[l].selfattn(v, gradval, x[:, -1:, :], x, x)
                v, gradval, out_temp = self.layers[l].attention(v, gradval, x, encoding[l], encoding[l], mask_src)
                hiddens[l + 1][:, :, t] = self.layers[l].feedforward(out_temp).view(B, W, C)
...
class Transformer(nn.Module):
    ...
    def __init__(self, src=None, trg=None, args=None):
        ...
        if args.dataset != "mscoco":
            ...
            if args.res_type == 'adaptive':
                self.encoder = EncoderAdaptive(src, args)
            elif args.res_type == 'momentum':
                self.encoder = EncoderMomentum(src, args)
            else:
                self.encoder = Encoder(src, args)
                
            if args.res_type == 'adaptive':
                self.decoder = DecoderAdaptive(trg, args)
            elif args.res_type == 'momentum':
                self.decoder = DecoderMometum(trg, args)
            else:
            ...
            if args.res_type == 'adaptive':
                self.decoder = DecoderAdaptive(mscoco_dataset, args)
            elif args.res_type == 'momentum':
                self.decoder = DecoderMomentum(mscoco_dataset, args)
            else:
...
class FastTransformer(Transformer):
    ...
    def __init__(self, src=None, trg=None, args=None):
        ...
        for ni in range(args.num_decs):
            if args.res_type == 'adaptive':
                self.decoder.append(DecoderAdaptive(trg, args,
                                        causal=False,
                ...
                self.decoder.append(DecoderMomentum(trg, args,
                                        causal=False,
                                        positional=args.positional,
                                        diag=args.diag,
                                        out=self.encoder.out if args.share_embed_enc_dec1 and ni == 0 else None)
    ...
    def cost(self, targets, target_mask, out=None, iter_=0, return_logits=False):
        # get loss in a sequence-format to save computational time.
        targets, out = prepare_cost(targets, out, target_mask.bool())
