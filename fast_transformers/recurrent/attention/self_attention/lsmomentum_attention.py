...
class RecurrentLSMomentumAttention(Module):
    ...
    def __init__(self, query_dimensions, mu, stepsize, feature_map=None, eps=1e-6,
                 event_dispatcher=""):
        super(RecurrentLSMomentumAttention, self).__init__()
        self.feature_map = (
        ...
        self.mu = mu
        self.stepsize = torch.nn.Parameter(torch.as_tensor(stepsize))
        self.stepsize.requires_grad = True
...
RecurrentAttentionRegistry.register(
    "lsmomentum-linear", RecurrentLSMomentumAttention,
    [
        ("query_dimensions", Int),
        ("mu", Float),
        ("stepsize", Float),
