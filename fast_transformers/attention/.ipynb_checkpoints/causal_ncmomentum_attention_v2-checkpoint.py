...
class CausalNCMomentumAttention_v2(Module):
    ...
    def __init__(self, query_dimensions, stepsize, feature_map=None, eps=1e-6,
                 event_dispatcher=""):
        super(CausalNCMomentumAttention_v2, self).__init__()
        self.feature_map = (
    ...
    def forward(self, queries, keys, values, attn_mask, query_lengths,
        ...
        if not attn_mask.lower_triangular:
            raise RuntimeError(("CausalNCMomentumAttention_v2 only supports full "
...
#         mu = (1.0 - torch.sqrt(self.stepsize * (torch.norm((KV[:,1:,:,:,:] - KV[:,:-1,:,:,:]).reshape(-1,L-1), dim=0) / torch.norm(KV[:,:-1,:,:,:].reshape(-1,L-1), dim=0))))**2
        mu = (1.0 - torch.sqrt((torch.norm((KV[:,1:,:,:,:] - KV[:,:-1,:,:,:]).reshape(-1,L-1), dim=0) / torch.norm(KV[:,:-1,:,:,:].reshape(-1,L-1), dim=0))))**2
        mu = torch.clamp(mu, min=0.0, max=0.9999)
        
        # Form the vector of coefficent momentum_weights
        momentum_weight = self.stepsize
        Kw = K.clone()
        for j in range(1,L+1):
            # import pdb; pdb.set_trace()
            Kw[:,-j,:,:] = K[:,-j,:,:] * momentum_weight
            if j < L:
...
AttentionRegistry.register(
    "causal-ncmomentum-v2", CausalNCMomentumAttention_v2,
    [
