...
class RecurrentTransformerAdaptiveEncoderLayer(Module):
    ...
    def forward(self, x, v, gradval, state=None, memory=None):
        ...
        gradvaly, state = self.attention(x, x, x, state)
        gradvaly = self.dropout(gradvaly)
        
        if self.adaptive_type == "fr":
            #Fletcher–Reeves
            res_mu = (torch.norm(gradvaly, dim=1, keepdim=True)/torch.norm(gradval, dim=1, keepdim=True))**2
        elif self.adaptive_type == "pr":
            #Polak–Ribière
            res_mu = torch.sum(gradvaly * (gradvaly - gradval), dim=1, keepdim=True)/(torch.norm(gradval, dim=1, keepdim=True)**2)
        elif self.adaptive_type == "hs":
            #Hestenes-Stiefel
            if torch.all(v == torch.zeros_like(x)):
                res_mu = torch.sum(gradvaly * (gradvaly - gradval), dim=1, keepdim=True)/torch.sum(gradvaly - gradval, dim=1, keepdim=True)
            else:
                res_mu = torch.sum(gradvaly * (gradvaly - gradval), dim=1, keepdim=True)/torch.sum(v * (gradvaly - gradval), dim=1, keepdim=True)
            ...
            if torch.all(v == torch.zeros_like(x)):
                ...
                res_mu = (1.0 - torch.sqrt(torch.norm(gradvaly - gradval, dim=1, keepdim=True)/torch.norm(v, dim=1, keepdim=True)))**2
        ...
        return self.norm2(x+y), vy, gradvaly, state
...
class RecurrentTransformerAdaptiveEncoder(Module):
    ...
    def __init__(self, layers, norm_layer=None, event_dispatcher=""):
        super(RecurrentTransformerAdaptiveEncoder, self).__init__()
    ...
    def forward(self, x, state=None, memory=None):
        ...
        for i, layer in enumerate(self.layers):
            x, v, gradval, s = layer(x, v, gradval, state[i])
            state[i] = s
