...
class RecurrentNormgradAttention(Module):
    ...
    def __init__(self, query_dimensions, mu, stepsize, feature_map=None, eps=1e-6,
                 event_dispatcher=""):
        super(RecurrentNormgradAttention, self).__init__()
        self.feature_map = (
    ...
    def forward(self, query, key, value, state=None, memory=None):
        ...
        if K.grad_fn is not None or value.grad_fn is not None:
            ...
            Pi = self.mu * Pi - self.stepsize * Ui / torch.norm(Ui, dim=(2,3), keepdim=True)
            Si = Si - Pi
            ...
            Ui = torch.einsum("nhd,nhm->nhdm", K, value)
            Pi *= self.mu
            Pi -= self.stepsize * Ui / torch.norm(Ui, dim=(2,3), keepdim=True)
            Si -= Pi
...
RecurrentAttentionRegistry.register(
    "normgrad-linear", RecurrentNormgradAttention,
    [
        ("query_dimensions", Int),
        ("mu", Float),
        ("stepsize", Float),
