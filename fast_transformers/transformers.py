...
class TransformerAdaptiveEncoderLayer(Module):
    ...
    def __init__(self, attention, d_model, d_ff=None, dropout=0.1,
        ...
        super(TransformerAdaptiveEncoderLayer, self).__init__()
        ...
        self.is_resw = is_resw
        
        # for Wx + A(x)
        if self.is_resw:
            self.linres1 = Linear(d_model, d_model)
            self.linres2 = Linear(d_model, d_model)

    def forward(self, x, v, gradval, attn_mask=None, length_mask=None):
        ...
        gradvaly = self.dropout(self.attention(
            x, x, x,
            attn_mask=attn_mask,
        ...
        if self.is_resw:
            x = self.linres1(x)  - self.res_stepsize * self.linres2(vy)
        else:
        ...
        return self.norm2(x+y), vy, gradvaly


class TransformerMomentumEncoderLayer(Module):
    """Self attention and feed forward network with skip connections.
    ...
    """
    def __init__(self, attention, d_model, d_ff=None, dropout=0.1,
                 activation="relu", event_dispatcher="", res_stepsize=1.0, res_mu=0.0, is_resw=False):
        super(TransformerMomentumEncoderLayer, self).__init__()
        d_ff = d_ff or 4*d_model
        self.attention = attention
        self.linear1 = Linear(d_model, d_ff)
        self.linear2 = Linear(d_ff, d_model)
    ...
    def forward(self, x, v, attn_mask=None, length_mask=None):
        """Apply the transformer encoder to the input x.
        ...
        # Run self attention and add it to the input
        vy = self.res_mu * v - self.dropout(self.attention(
            x, x, x,
            attn_mask=attn_mask,
            query_lengths=length_mask,
            key_lengths=length_mask
...
class TransformerAdaptiveEncoder(Module):
    ...
    def __init__(self, layers, norm_layer=None, event_dispatcher=""):
        super(TransformerAdaptiveEncoder, self).__init__()
    ...
    def forward(self, x, attn_mask=None, length_mask=None):
        ...
        for layer in self.layers:
            x, v, gradval = layer(x, v, gradval, attn_mask=attn_mask, length_mask=length_mask)
...
class TransformerMomentumEncoder(Module):
    """TransformerEncoder is little more than a sequence of transformer encoder
    layers.
    ...
    def forward(self, x, attn_mask=None, length_mask=None):
        ...
        # Apply all the transformers
        for layer in self.layers:
            x, v = layer(x, v, attn_mask=attn_mask, length_mask=length_mask)
