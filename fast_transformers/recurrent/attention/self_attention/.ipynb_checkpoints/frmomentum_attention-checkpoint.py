...
class RecurrentFRMomentumAttention_v2(Module):
    ...
    def __init__(self, query_dimensions, stepsize, feature_map=None, eps=1e-6,
                 event_dispatcher=""):
        super(RecurrentFRMomentumAttention_v2, self).__init__()
        self.feature_map = (
    ...
    def forward(self, query, key, value, state=None, memory=None):
        ...
        if state is None:
            ...
            Siprev, Zi, Pi, Uiprev, Siprev2 = state
...
#                 mu = (1.0 - torch.sqrt(self.stepsize * torch.norm((Ui - Uiprev), dim=(2,3), keepdim=True) / torch.norm((Siprev - Siprev2), dim=(2,3), keepdim=True)))**2
                mu = (1.0 - torch.sqrt(self.stepsize * torch.norm((Ui - Uiprev), dim=(2,3), keepdim=True) / torch.norm(Uiprev, dim=(2,3), keepdim=True)))**2
                mu = torch.clamp(mu, min=0.0, max=0.9999)
                # mu = torch.norm(Ui, dim=(2,3), keepdim=True) / torch.norm(Uiprev, dim=(2,3), keepdim=True)
                # mu = torch.norm(Ui.reshape(-1), dim=0, keepdim=True) / torch.norm(Uiprev.reshape(-1), dim=0, keepdim=True)
...
RecurrentAttentionRegistry.register(
    "causal-ncmomentum", RecurrentFRMomentumAttention_v2,
    [
        ("query_dimensions", Int),
        ("stepsize", Float),
        ("feature_map", Optional(Callable)),
