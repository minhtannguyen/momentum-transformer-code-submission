...
class SequencePredictor(torch.nn.Module):
    def __init__(self, d_model, sequence_length, n_classes,
                 ...
                 bits=32, rounds=4,
                 chunk_size=32, masked=True, mu=0.0, stepsize=1.0):
    ...
    def get_stepsize(self):
        ...
        for i in range(len(self.transformer.layers)):
            stepsize_list.append(self.transformer.layers[i].attention.inner_attention.stepsize)
    ...
    def forward(self, x):
        ...
        for x_t in torch.unbind(x, dim=1):
            out_t, state = self.transformer(x_t, state) # out_t: [64, 1, 256]
            outputs.append(out_t)
            
        y_hat = torch.stack(outputs, dim=1) # y_hat: [64, 128, 256]
...
def train(model, optimizer, dataloader, device, epoch, writer=None, logger=None):
    model.train()
    stats = EpochStats(["accuracy"])
    for i, (x, y, m) in zip(range(100), dataloader):
        iter_indx = (epoch - 1) * 100 + i
        x = x.to(device).t()
        y = y.to(device).t()
        m = m.to(device).t()
        optimizer.zero_grad()
        ...
        writer.add_scalars('train_loss', {'loss': l}, iter_indx)
        writer.add_scalars('train_acc', {'acc': acc}, iter_indx)
        
        logger.file.write('\nIter: [%d] Train Loss: %f' % (iter_indx, l))
        logger.file.write('\nIter: [%d] Train Acc: %f' % (iter_indx, acc))
...
def main(argv=None):
    ...
    title = 'copy-' + args.attention_type
    ...
    for e in range(start_epoch, args.epochs+1):
        train(model, optimizer, train_loader, device, epoch=e, writer=writer, logger=logger)
        test_loss, test_acc = evaluate(model, test_loader, device)
            
        if args.continue_from:
            if args.save_to and (test_acc > best_acc):
                save_model(os.path.join(args.save_to, 'model_best_acc_cont.pth.tar'), model, optimizer, e)

            if args.save_to and (test_loss < best_loss):
                save_model(os.path.join(args.save_to, 'model_best_loss_cont.pth.tar'), model, optimizer, e)
        else:
            if args.save_to and (test_acc > best_acc):
                save_model(os.path.join(args.save_to, 'model_best_acc.pth.tar'), model, optimizer, e)

            if args.save_to and (test_loss < best_loss):
                save_model(os.path.join(args.save_to, 'model_best_loss.pth.tar'), model, optimizer, e)
        
        best_loss = min(test_loss, best_loss)
        best_acc = max(test_acc, best_acc)
        writer.add_scalars('test_loss', {'loss': test_loss}, e)
        writer.add_scalars('test_acc', {'acc': test_acc}, e)

        logger.file.write('\nEpoch: [%d] Test Loss: %f Best Loss: %f' % (e, test_loss, best_loss))
        logger.file.write('\nEpoch: [%d] Test Acc: %f Best Acc: %f' % (e, test_acc, best_acc))
