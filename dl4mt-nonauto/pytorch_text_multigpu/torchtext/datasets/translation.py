...
class TranslationDataset(data.Dataset):
    ...
    def __init__(self, path, exts, fields, **kwargs):
        ...
        if not 'iwslt' in src_path and not 'iwslt' in trg_path:
            sys.exit("code is not ready for NON iwslt")

        is_train = 'train' in src_path and 'train' in trg_path
        train_examples_path = "/misc/vlgscratch2/ChoGroup/mansimov/iwslt_torchtext/train_examples.pkl"

        if is_train and os.path.isfile(train_examples_path):
            examples = pickle.load(open(train_examples_path, "rb"))
        else:
            examples = []
            ...
            if is_train:
                pickle.dump(examples, open(train_examples_path, "wb"))
