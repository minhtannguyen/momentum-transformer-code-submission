...
from fast_transformers.builders import TransformerMomentumEncoderBuilder
...
class SequencePredictor(torch.nn.Module):
    def __init__(self, d_model, sequence_length, n_classes,
                 ...
                 bits=32, rounds=4,
                 chunk_size=32, masked=True, mu=0.0, stepsize=1.0, delta=0.0001, res_stepsize=1.0, res_mu=0.0):
        ...
        self.transformer = TransformerMomentumEncoderBuilder.from_dictionary(
            self.builder_dict,
            strict=True
        ).get()
...
def main(argv=None):
    ...
    for e in range(start_epoch, args.epochs+1):
        ...
        # time1 = time.time()
        # print('train time per epoch = %f seconds'%(time1 - time0))
