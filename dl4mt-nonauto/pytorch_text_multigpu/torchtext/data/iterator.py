...
class Iterator(object):
    ...
    def __init__(self, dataset, batch_size, sort_key=None, device=None,
                 ...
                 sort_within_batch=None, num_gpus=1):
    ...
    def create_batches(self):
        if self.sort:
            self.batches = batch(self.data(), self.batch_size,
                                 self.batch_size_fn, num_gpus=self.num_gpus)
        else:
...
def batch(data, batch_size, batch_size_fn=lambda new, count, sofar: count,
    ...
    # MAKE SURE THAT BATCH ALWAYS HAS examples % num_gpus == 0
    # AS WELL AS NUM EXAMPLES IN MINIBATCH >= num_gpus
    minibatch, size_so_far = [], 0
    for ex in data:
        minibatch.append(ex)
        size_so_far = batch_size_fn(ex, len(minibatch), size_so_far)
        if size_so_far == batch_size:
            if len(minibatch) >= num_gpus and len(minibatch) % num_gpus == 0:
                num_examples = len(minibatch)
                assert (num_examples % num_gpus == 0)
                yield minibatch
                minibatch, size_so_far = [], 0
            elif len(minibatch) >= num_gpus and len(minibatch) % num_gpus != 0:
                num_examples = len(minibatch) - (len(minibatch) % num_gpus)
                assert (num_examples % num_gpus == 0)
                yield minibatch[:num_examples]
                # rest of minibatch will be used in next iteration
                minibatch, size_so_far = minibatch[num_examples:], 0
                for remain_i, remain_ex in enumerate(minibatch[num_examples:]):
                    size_so_far = batch_size_fn(remain_ex, remain_i+1, size_so_far)

        elif size_so_far > batch_size:
            if len(minibatch[:-1]) >= num_gpus and len(minibatch[:-1]) % num_gpus == 0:
                num_examples = len(minibatch[:-1])
                assert (num_examples % num_gpus == 0)
                yield minibatch[:-1]
                minibatch, size_so_far = minibatch[-1:], batch_size_fn(ex, 1, 0)
            elif len(minibatch[:-1]) >= num_gpus and len(minibatch[:-1]) % num_gpus != 0:
                num_examples = len(minibatch[:-1]) - (len(minibatch[:-1]) % num_gpus)
                assert (num_examples % num_gpus == 0)
                yield minibatch[:num_examples]
                # rest of minibatch including the last one will be used in next iteration
                minibatch, size_so_far = minibatch[num_examples:], 0
                for remain_i, remain_ex in enumerate(minibatch[num_examples:]):
                    size_so_far = batch_size_fn(remain_ex, remain_i+1, size_so_far)
    ...
    if minibatch:
        if len(minibatch) >= num_gpus and len(minibatch) % num_gpus == 0:
            ...
            # corner case
            while len(minibatch) < num_gpus or len(minibatch) % num_gpus != 0:
                # randomly select sample and append it
                minibatch.append(deepcopy(minibatch[random.randint(0, len(minibatch)-1)]))
            yield minibatch

def pool(data, batch_size, key, batch_size_fn=lambda new, count, sofar: count,
         random_shuffler=None, num_gpus=1):
    ...
    for p_total in batch(data, batch_size * 100, batch_size_fn, num_gpus=num_gpus):
        p_total_list = list(p_total)
        assert (len(p_total_list) % num_gpus == 0)
        p_batch = batch(sorted(p_total, key=key), batch_size, batch_size_fn, num_gpus=num_gpus)
